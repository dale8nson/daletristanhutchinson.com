...
const Shoji = ({scale=Vec3(1,1,1), position, greyOffset=0.0, rotation=new THREE.Euler(0,0,0)}) => {
  ...
  const Lattice = ({position}) => {
    const hLatticeArray = Array(13).fill(null).map((_, index) => {
      return (
        <Strut key={index} position={Vec3(position.x + 0.04 * scale.x, position.y - 0.35 -  (index + 1) * 0.4 * scale.y, position.z * scale.z)} scale={Vec3(0.00197 * scale.x, 0.000075 * scale.y, 1 * scale.z)}  />
      );
    })
